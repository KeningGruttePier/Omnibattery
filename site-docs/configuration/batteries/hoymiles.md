...
The 4020 expansion battery packs increase stored energy but do not increase the
main unit's 2000 W battery charge/discharge ceiling. Verify the capacity shown
in the limits step when expansion packs are installed.
...
4020 X, for example, uses a `4.02 kWh` capacity and a maximum `2000 W` profile
instead of inheriting the MS-A2's `2.24 kWh` and `1000 W`; a lower limit
advertised by that individual device is preserved.
